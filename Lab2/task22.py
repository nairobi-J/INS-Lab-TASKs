...
english_freq = "etaoihnsrdlbufmwgycpvkxjqz"
...
def break_substitution_cipher(cipher):
    ...
    common_letters = [c for c, _ in sorted_freq]
    ...
    for c in cipher:
        if c.lower() in mapping:
            new_char = mapping[c.lower()]
            decrypted += new_char.upper() if c.isupper() else new_char
        else:
            decrypted += c
